        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((s, i) => (
            <motion.article
              ...
              className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-md will-change-transform"
              aria-labelledby={`${s.title.replace(/\s+/g, '-')}-title`}
            >
              ...
              <div className="relative flex flex-col gap-3">
                ...
                <h3 id={`${s.title.replace(/\s+/g, '-')}-title`} className="text-lg font-semibold text-white">
                  {s.title}
                </h3>
                ...
                <button
                  type="button"
                  className="mt-2 inline-flex w-fit items-center gap-2 rounded-full bg-gradient-to-r from-cyan-500 to-purple-600 px-4 py-2 text-xs font-semibold text-white shadow-[0_0_12px_rgba(59,130,246,0.25)] transition-transform group-hover:scale-[1.02] focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
